"""
func_2d/function.py - MedSAM2 2D 训练/验证函数 (BSPT集成版)

修改内容:
1. 集成BSPT模块 (DAFA, PFBE, BAAM, ULA, DSA)
2. 使用HBALLoss替代原有损失函数
3. 在image_embed上应用BSPT增强
"""
import os
import numpy as np
import torch
...
# ★★★ BSPT导入 ★★★
from func_2d.bspt_utils import create_bspt_modules, apply_bspt_to_features, create_bspt_loss
...
def init_bspt_modules(args, net, GPUdevice):
    """
    初始化BSPT模块（只执行一次）
    """
    if getattr(args, 'use_bspt', False) and not hasattr(args, '_bspt_modules'):
        print("\n" + "=" * 60)
        print("🚀 Initializing BSPT Modules...")
        print("=" * 60)

        # 创建模块
        args._bspt_modules = create_bspt_modules(net.hidden_dim, args)

        # 移动到GPU
        for name, m in args._bspt_modules.items():
            args._bspt_modules[name] = m.to(GPUdevice)

        # 创建BSPT损失
        args._bspt_loss = create_bspt_loss(args, GPUdevice)

        # 设置模块为训练模式
        for m in args._bspt_modules.values():
            m.train()

        print("✅ BSPT modules initialized and moved to GPU")
        print("=" * 60 + "\n")

    return getattr(args, '_bspt_modules', None), getattr(args, '_bspt_loss', None)
...
def train_sam(args, net: nn.Module, optimizer, train_loader, epoch, writer, lossfunc=None):
    ...
    # ★★★ 初始化BSPT模块 ★★★
    bspt_modules, bspt_loss = init_bspt_modules(args, net, GPUdevice)
    ...
    # Loss 函数 (如果使用BSPT，优先使用bspt_loss)
    if bspt_loss is not None:
        print(f"[Train] Using BSPT HBALLoss")
    elif lossfunc is None:
        loss_type = getattr(args, 'loss_type', 'bce')
        lossfunc = get_loss_function(loss_type, pos_weight=2.0, device=GPUdevice)
        print(f"[Train] Using {lossfunc.__class__.__name__}")
    ...
    if epoch == 0:
        ...
        print(f"[Train] BSPT enabled: {bspt_modules is not None}")
...
def validation_sam(args, val_loader, epoch, net: nn.Module, clean_dir=True, lossfunc=None):
    ...
    # ★★★ 获取BSPT模块（如果已初始化）★★★
    bspt_modules = getattr(args, '_bspt_modules', None)
    bspt_loss = getattr(args, '_bspt_loss', None)
    ...
    # Loss 函数
    if bspt_loss is None and lossfunc is None:
        loss_type = getattr(args, 'loss_type', 'bce')
        lossfunc = get_loss_function(loss_type, pos_weight=2.0, device=GPUdevice)
    ...
    with tqdm(total=n_val, desc='Validation round', unit='batch', leave=False) as pbar:
        for ind, pack in enumerate(val_loader):
            ...
            with torch.no_grad():
                ...
                # ★★★ 使用BSPT损失或原有损失 ★★★
                if bspt_loss is not None:
                    loss, _ = bspt_loss(pred, masks, epoch=epoch)
                    total_loss += loss
                else:
    ...
    bspt_status = "BSPT ✓" if bspt_modules is not None else "Baseline"
    print(f"\n{'=' * 60}")
    print(f"Epoch {epoch} 验证结果 [{bspt_status}] (image_size={args.image_size})")
    print(f"{'=' * 60}")
