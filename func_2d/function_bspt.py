"""
func_2d/function_bspt.py - MedSAM2 2D 训练/验证函数 (BSPT集成版)
=================================================================
基于 function_improved.py，集成 BSPT 模块

核心改动:
1. 在特征提取后应用 BSPT 模块
2. 使用 HBALLoss 替代原有损失
3. 保持与原有代码完全相同的结构
"""
...
def train_sam_with_scheduler(args, net: nn.Module, optimizer, train_loader, epoch, writer,
    ...
    if epoch == 0:
        ...
        print(f"[Train] BSPT 模块: {'启用' if bspt_modules else '禁用'}")
        if scheduler:
    ...
    with tqdm(total=len(train_loader), desc=f'Epoch {epoch}', unit='img') as pbar:
        for ind, pack in enumerate(train_loader):
            ...
            if max_grad_norm > 0:
                ...
                # 裁剪 BSPT 模块梯度
                if bspt_modules is not None:
                    for module in bspt_modules.values():
...
def validation_sam(args, val_loader, epoch, net: nn.Module, clean_dir=True):
    ...
    if abnormal_stats.abnormal > 0 and epoch % 10 == 0:  # 每10个epoch打印详情
